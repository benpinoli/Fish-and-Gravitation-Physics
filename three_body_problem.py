...
def main():
    ...
    # get random seed
    # random.seed(38) unneeded



    s = School(N=3, iterations=iterations, mvmt_type='attraction', mass=.001)
    #parameterize the bounds
    v = .5
    r = 100
    # 1) preset the positions of the two particles
    s.r = np.array([[x_dim/3, y_dim/2],
                    [-x_dim/4, y_dim/4],
                    [0, -y_dim/4]])
    s.v = np.array([[4, -6],
                    [1, 4],
    ...
    animation = anim.FuncAnimation(
        ...
        interval = 20 #number of milliseconds per frame
        )
